...
//-------------------------------------------------------
    ...
    kernel_pbarx.setKernelPbarX(momtable); //setKernelPbarX(momtable);

    lfield<double,9> kernel_pbary(cnfg->Nxl, cnfg->Nyl);
    kernel_pbary.setToZero();
    kernel_pbary.setKernelPbarY(momtable); //setKernelPbarY(momtable);
...
//		std::cout << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << std::endl;
    ...
    const std::string file_name = "test_momentum_evolution";
