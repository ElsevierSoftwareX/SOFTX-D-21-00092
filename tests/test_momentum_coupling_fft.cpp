...
//    fourier->init1D(mpi->getRowComm(), mpi->getColComm());    
    ...
    lfield<double,1> coupling(Nx,Ny);
	...
	for(int y = 0; y < Ny; y++){
		...
		coupling.u[i] = std::complex<double>( tmp2 / log( pow( tmp + pow((momtable->phat2(i)*Nx*Ny)/6.0/6.0,1.0/0.2) , 0.2) ) , 0.0);

	}

    }

    fourier2->execute2D(&coupling, 0);
	...
	for(int y = 0; y < Ny; y++){
		...
		printf("%i %i %f %f\n", x, y, coupling.u[i].real(), coupling.u[i].imag());
	}
